<?php 
    ...
    if(isset($_POST['add_airline'])){
      ...
      if(!empty($_FILES["image"]["name"])){
        ...
        if(move_uploaded_file($tempname, $folder)){
          mysqli_query($con, "INSERT INTO airlines (airline_name, iata_code, icao_code, airline_description, airline_img)
                        VALUES ('$airline_name','$iata_code','$icao_code','$airline_description','$airline_img')");
                        echo "<script>window.location='../airline.php?msg=img_success'</script>";
        }else{
          echo "<script>window.location='../airline.php?msg=img_err'</script>";
        }
      }else{
        mysqli_query($con, "INSERT INTO airlines (airline_name, iata_code, icao_code, airline_description)
                          ...
                          echo "<script>window.location='../airline.php?msg=no_img'</script>";
